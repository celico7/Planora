    ...
    <div class="flex gap-6 overflow-x-auto py-4">
        @forelse($sprint->epics as $epic)
            <div class="min-w-[18rem] bg-[#f5f7fa] rounded-lg shadow p-3 flex-shrink-0">
                <h2 class="font-bold mb-2 text-primary">{{ $epic->nom }}</h2>
                    ...
                    @forelse($epic->tasks as $task)
                        <div class="bg-white shadow rounded p-2 text-sm flex flex-col gap-1">
                            ...
                            <div class="flex gap-2 items-center mt-1">
                                <span class="px-2 py-1 rounded bg-primary/20 text-primary text-xs font-semibold">{{ ucfirst($task->statut) }}</span>
                                <span class="px-2 py-1 rounded bg-secondary/10 text-secondary text-xs">{{ ucfirst($task->priorite) }}</span>
               ...
               <div class="p-2 mt-4 rounded bg-gray-200 shadow transition-colors duration-200 group hover:bg-gray-100 z-10">
                <div class="flex items-center gap-3">
                    <i class="bi bi-plus-circle-fill text-gray-600 group-hover:text-primary text-lg"></i>
                    <a href="{{ route('projects.sprints.epics.tasks.create', [
                            'project' => $project->id,
                            'sprint' => $sprint->id,
                            'epic' => $epic->id]) }}"
                    class="block text-sm font-medium text-secondary group-hover:text-primary">
